...
inline bool IsTextMime(const std::string& mime_type) {
  ...
  return mime_type == "application/json" || mime_type == "application/javascript" ||
...
inline std::string GetExtension(const std::string& mime_type) {
  static const std::unordered_map<std::string, std::string> ext_map = {
      ...
      {"text/csv", ".csv"},          {"text/markdown", ".md"},    {"application/javascript", ".js"},
      {"application/json", ".json"}, {"application/xml", ".xml"}, {"application/pdf", ".pdf"},
      {"application/zip", ".zip"},   {"application/gzip", ".gz"}, {"application/wasm", ".wasm"},
      {"image/png", ".png"},         {"image/jpeg", ".jpg"},      {"image/gif", ".gif"},
      {"image/svg+xml", ".svg"},     {"image/webp", ".webp"},     {"image/x-icon", ".ico"},
      {"audio/mpeg", ".mp3"},        {"audio/wav", ".wav"},       {"audio/ogg", ".ogg"},
      {"video/mp4", ".mp4"},         {"video/webm", ".webm"},     {"font/woff", ".woff"},
