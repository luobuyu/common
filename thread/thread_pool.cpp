...
const ThreadPool::Stats& ThreadPool::GetStats() const {
  return m_stats;
}
...
uint64_t ThreadPool::GetMaxWaitTimeMs() const {
  return m_max_wait_time_ms;
}
...
void ThreadPool::Worker() {
  while (true) {
    ...
    // 超时丢弃检查
    if (m_max_wait_time_ms > 0 && wait_time > m_max_wait_time_ms * 1000) {  // 转换为微秒
