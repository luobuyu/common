...
namespace logger
{
    ...
    void logger::FormatItem::set_fmt(const std::string &fmt)
    {
    }
    std::string logger::FormatItem::get_fmt()
    {
        return std::string();
    }
    void logger::LoggerFormat::parser_patterm()
    {
        ...
        for (int i = 0; i < m_pattern.size();)
        {
            if(m_pattern[i] == '%') {
                ...
                else
                {
                    ...
                    if(is_pre_other)
                        m_format_items.back()->set_fmt(std::move(m_format_items.back()->get_fmt() + val));
    ...
    void logger::OtherFormatItem::set_fmt(const std::string &fmt)
    {
        m_other_fmt = fmt;
    }
    std::string logger::OtherFormatItem::get_fmt()
    {
        return m_other_fmt;
    }
