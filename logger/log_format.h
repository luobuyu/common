...
namespace logger
{
    ...
    public:
        ...
        virtual void set_fmt(const std::string &fmt);
        virtual std::string get_fmt();
    ...
    public:
        ...
        void format(std::ostream &os, const logger::LogEvent &log_msg) override;
        void set_fmt(const std::string &fmt) override;
        std::string get_fmt() override;
    ...
    public:
        ...
        // %n : 换行符[\r\n]       NewLineFormatItem
        LoggerFormat(const std::string &pattern = "%d{%Y-%m-%d %H:%M:%S}%T[%L]%T[%p,%t,%c]%T[%M]%T[%F:%f:%l]%T%m%n");
    ...
    {
    public:
        static std::shared_ptr<FormatItem> createFormatItem(const char &key, const std::string &val = std::string());
