...
namespace logger 
{
    logger::FileSink::FileSink(const std::string &file_path, uint32_t max_size, dry::clock::duration rotate_inteval)
    : m_file_path(file_path), m_no(0),
    m_max_size(max_size),
    ...
    void StdoutSink::sink(const logger::LogEvent &log_event, LoggerFormat::LoggerFormatPtr &log_format)
    {
