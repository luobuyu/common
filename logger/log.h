...
class Logger {
 ...
 protected:
  ...
  Logger(std::vector<LogSink::LogSinkPtr> sinks, LoggerFormat::LoggerFormatPtr fmt);
...
class SyncLogger : public Logger {
 public:
  ...
  /// 带参构造：构造即可用
  SyncLogger(std::vector<LogSink::LogSinkPtr> sinks, LoggerFormat::LoggerFormatPtr fmt);

  void Log(LogEvent log_event) override;
  ~SyncLogger() override = default;

 private:
  std::mutex m_mtx;  ///< 串行化同步日志写入
};
...
class AsyncLogger : public Logger {
 public:
  ...
  AsyncLogger(std::vector<LogSink::LogSinkPtr> sinks, LoggerFormat::LoggerFormatPtr fmt,
              int queue_size = 8192,
